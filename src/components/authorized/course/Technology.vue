<template>
  <div id="tech" ref="tech"
    ...
    <div class="flex flex-col justify-end ml-6 md:items-center md:flex-row">
      ...
      <progress class="w-28 md:w-40 lg:w-48 progress progress-success" :value="value" :max="numberOfLessons"></progress>
      ...
      <router-link to="/frontend-path/html5">
        <Button :disabled="disabled" :class="{ 'opacity-10': disabled }"
          class="mt-4 text-sm text-white md:mt-0 bg-main-1 md:text-xl">{{ buttonText }}</Button>
      </router-link>
    </div>
  </div>
</template>
